    ...
    for (lng in 1:iterNG) { # lng = 1
      ...
      doparCollect <- suppressPackageStartupMessages(
        ...
        foreach(i = 1:iterPar) %dorng% { # i = 1
          ...
          if (hyper_fixed == FALSE) {
            mod_out <- model_refit(x_train, y_train, lambda = lambda, lower.limits, upper.limits, InputCollect$intercept_sign, regression_methodology)
          } else {
            mod_out <- model_refit(x_train, y_train, lambda = lambda_fixed[i], lower.limits, upper.limits, InputCollect$intercept_sign, regression_methodology)
